...
/**
 *  此案例是标准工厂方法模式，用于解决一对多静态简单工厂模式中代码逻辑复杂的问题[simple.factory2]
 *
 *      标准工厂方法模式是类的创建模式，又叫做虚拟构造子(Virtual Constructor)模式或者多态性工厂（Polymorphic Factory）模式。
 *      标准工厂方法模式的用意是定义一个创建产品对象的工厂接口，将实际创建工作推迟到子类中。
 *
 *  结构：
 *                      工厂接口                                     实体类
 *                ┌──────────────────┐
 ...
 *      2、标准工厂方法模式简化后可以变得成简单工厂模式。设想如果非常确定一个系统只需要一个具体工厂类，那么不妨把抽象工厂类合并到具体工厂类中去。
 *         由于只有一个具体工厂类，所以不妨将工厂方法改为静态方法，这时候就得到了简单工厂模式。
 *      3、如果系统需要加入一个新的Person子类，那么所需要的就是向系统中加入一个继承UserFactory类的子类对应的Person的新类即可。
