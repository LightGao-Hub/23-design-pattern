 ...
 *
 *  总结：
 *      1、标准工厂模式和简单工厂模式在结构上的不同很明显。工厂方法模式的核心是一个抽象工厂类，而简单工厂模式是把核心放在一个具体类的静态函数上。
 *      2、标准工厂模式简化后可以变得成简单工厂模式。设想如果非常确定一个系统只需要一个具体工厂类，那么不妨把抽象工厂类合并到具体工厂类中去。
 ...
 *
 *  缺点：标准工厂模式虽然符合开闭原则，但具体工厂类可以使用单例模式代替，这样既安全又方便使用，参考[perfect包]
 *
 * Author: GL
 * Date: 2021-11-25
 */
